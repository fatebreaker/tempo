...
def compute_tempo_outcome_advantage(
    ...
    """
    scores = token_level_rewards.sum(dim=-1)
    response_scores = deepcopy(scores)
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                ...
                scores[i] = scores[i] - id2mean[index[i]]
        scores = scores.unsqueeze(-1) * response_mask
    token_level_scores = deepcopy(scores)
    with torch.no_grad():
        bsz = scores.shape[0]
        for i in range(bsz):
            id2score[index[i]].append(scores[i])
        ...
        for i in range(bsz):
            previous_branch = id2index[index[i]]
            group_scores = [response_scores[k] for k in previous_branch]
            group_mean = torch.mean(torch.tensor(group_scores))
            group_std = torch.std(torch.tensor(group_scores))

            previous_branch = id2index[index[i]]
            for j in range(token_level_scores.shape[1]):
                if response_mask[i, j].item() == 0:
                    break
                branch = [k for k in previous_branch if responses[i, j] == responses[k, j]]
                ancestor_scores = [response_scores[k] for k in previous_branch]
                ancestor_mean = torch.mean(torch.tensor(ancestor_scores))
                # ancestor_std = torch.std(torch.tensor(ancestor_scores))
                branch_scores = [response_scores[k] for k in branch]
                branch_mean = torch.mean(torch.tensor(branch_scores))
                if norm_adv_by_std_in_grpo:
                    token_level_scores[i, j] = (response_scores[i] - group_mean + branch_mean  - ancestor_mean) / (group_std + epsilon)
                else:
                    token_level_scores[i, j] = response_scores[i] - group_mean + branch_mean  - ancestor_mean

                if len(previous_branch) == 1:
                    token_level_scores[i, j:] = token_level_scores[i, j]
                    break
                previous_branch = branch

    token_level_scores = token_level_scores * response_mask
    return token_level_scores, token_level_scores


# NOTE(sgm): this implementation only consider outcome supervision, where the reward is a scalar.
@register_adv_est(AdvantageEstimator.HEPO)  # or simply: @register_adv_est("grpo")
def compute_hepo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    entropys: torch.Tensor,
    index: np.ndarray,
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                ...
                scores[i] = scores[i] - id2mean[index[i]]
        scores = scores.unsqueeze(-1) * response_mask
        flat_entropys = entropys.flatten()
        k = int(response_mask.sum() * rho)
        topk_vals, topk_idx = torch.topk(flat_entropys, k)
        mask_flat = torch.zeros_like(flat_entropys, dtype=torch.float)
        mask_flat[topk_idx] = 1.0
        entropy_mask = mask_flat.view_as(entropys)
        mix_scores = entropy_mask * scores

    return mix_scores, mix_scores
