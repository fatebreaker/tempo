...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, config=None, tokenizer=None, epoch=None):
    ...
    elif adv_estimator == AdvantageEstimator.TIDEPO:
        # Initialize the mask for GRPO calculation
        tidepo_calculation_mask = data.batch["response_mask"]
        if multi_turn:
            # If multi-turn, replace the mask with the relevant part of loss_mask
            # Get length from the initial response mask
            response_length = tidepo_calculation_mask.size(1)
            # This mask is the one intended for GRPO
            gapo_calculation_mask = data.batch["loss_mask"][:, -response_length:]
        # Call compute_grpo_outcome_advantage with parameters matching its definition
        advantages, returns = core_algos.compute_tidepo_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            responses=data.batch["responses"],
            response_mask=tidepo_calculation_mask,
    ...
    elif adv_estimator == AdvantageEstimator.HEPO:
        # Initialize the mask for GRPO calculation
        hepo_calculation_mask = data.batch["response_mask"]
        if multi_turn:
            # If multi-turn, replace the mask with the relevant part of loss_mask
            # Get length from the initial response mask
            response_length = hepo_calculation_mask.size(1)
            # This mask is the one intended for GRPO
            hepo_calculation_mask = data.batch["loss_mask"][:, -response_length:]
        # Call compute_grpo_outcome_advantage with parameters matching its definition
        advantages, returns = core_algos.compute_hepo_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            entropys=data.batch["entropys"],
            response_mask=hepo_calculation_mask,
            index=data.non_tensor_batch["uid"],
            norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
            epoch=epoch
        )
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            ...
            AdvantageEstimator.HEPO,
            AdvantageEstimator.TIDEPO,
